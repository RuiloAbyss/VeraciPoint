...
export function Login() {
  ...
  const [isInitializing, setIsInitializing] = useState(true); // Nuevo estado para la carga inicial
      ...
      try {
        ...
        if (raw) {
          ...
          if (session.employeeId && session.activeTurnId) {
             ...
             if (isStillActive) {
                navigate("/dashboard");
                return; // Si hay sesión, navega directamente sin quitar la pantalla de carga
             } else {
        ...
        // Le damos un pequeño retraso estético de 1.2s para que se vea el logo antes de desvanecer
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isInitializing && (
          <motion.div 
            ...
            className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-surface-1/95 backdrop-blur-md"
          >
            <div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mb-6 shadow-sm" />
            <h1 className="text-3xl font-extrabold tracking-tight text-on-bg sm:text-4xl mb-2">
              Abarrotes <span className="text-primary">Janny</span>
            </h1>
            <h2 className="text-sm font-bold text-gray-500 tracking-widest uppercase animate-pulse">
              Iniciando sistema...
            </h2>
          </motion.div>
        )}
      ...
      {/* VISTA PRINCIPAL (Desvanecido en lugar de barrido) */}
      <motion.div
          ...
          <form onSubmit={handleSubmit} className="my-auto w-full space-y-5 lg:my-0 lg:pb-12">
            ...
            <div>
              ...
              <input
                ...
                disabled={dbStatus !== "active" || isLoading || isInitializing}
                autoComplete="off"
            ...
            <motion.button
              type="submit"
              disabled={isFormDisabled || isInitializing}
