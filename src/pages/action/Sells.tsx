...
// 1. Aceptamos string para poder manejar el input vacío o con punto "0."
interface CartItem {
  product: Product;
...
export function Sells() {
  ...
  const [errorModal, setErrorModal] = useState<{ isOpen: boolean; title: string; details: string }>({ 
    isOpen: false, 
    title: "", 
    details: "" 
  });
  ...
  // 2. Usamos Number() de forma segura para TypeScript
  ...
  const handleCheckout = async () => {
    ...
    try {
      const details: SaleDetailInput[] = cart.map(item => ({
        ...
        quantity: Number(item.quantity) || 0, // Evita enviar NaN o ""
  ...
  return (
    <motion.div className="absolute inset-0 flex flex-col p-2 sm:p-4 lg:p-5 gap-3 text-gray-900 z-20 bg-gray-50/50" initial={{ y: "100%" }} animate={{ y: "0%" }} exit={{ y: "100%" }} transition={{ duration: 0.28 }}>
      ...
      <header className="flex items-center justify-between rounded-xl bg-white border border-gray-200 px-4 py-3 shadow-sm shrink-0 flex-wrap gap-2">
        ...
        <button className="rounded-lg bg-gray-100 px-4 py-2 text-xs font-bold text-gray-400 shadow-sm cursor-not-allowed">
          Reportar Producto
        </button>
      </header>
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
        ...
        <div className="flex flex-col flex-1 lg:flex-[1.5] gap-3 min-h-0">
            <div className="flex items-center gap-2 rounded-xl bg-white border border-gray-200 p-2 shadow-sm shrink-0">
                <input 
                    ...
                    placeholder="🔍 Código o nombre... (Presiona Enter)" 
                    autoFocus
                    className="w-full rounded-lg bg-gray-50 px-3 py-2 text-sm outline-none border border-gray-200 focus:border-primary transition-colors" 
                />
            ...
            <section className="flex-col flex-1 rounded-2xl bg-gray-100/50 border border-gray-200 p-3 overflow-hidden flex">
        ...
        <div className="flex flex-col flex-1 lg:flex-1 w-full rounded-2xl bg-white border border-gray-200 shadow-sm min-h-0 overflow-hidden">
          <div className="bg-gray-50 p-4 border-b border-gray-200 flex justify-between items-center shrink-0">
            <h2 className="text-lg font-bold text-gray-900">Lista de Cobro</h2>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
            {cart.length === 0 ? (
                ...
                cart.map(item => {
                    ...
                    return (
                    <div key={item.product.id} className="flex flex-col gap-2 p-3 rounded-xl border border-gray-200 bg-gray-50/50 relative group">
                        <div className="flex justify-between items-start">
                            <div className="flex-1 pr-6">
                                <div className="flex items-center gap-2 mb-0.5">
                                    <p className="font-bold text-sm text-gray-900 leading-tight">{item.product.name}</p>
                                    <span className="text-[9px] bg-gray-200 text-gray-600 px-1.5 py-0.5 rounded-full uppercase font-bold tracking-wider">
                                        {isPieza ? 'PZA' : 'KG'}
                                    </span>
                                </div>
                                <p className="text-[10px] font-bold text-gray-400">P. Unit: ${(item.product.price || 0).toFixed(2)}</p>
                            </div>
                            <button onClick={() => removeFromCart(item.product.id)} className="absolute top-2 right-2 w-6 h-6 flex items-center justify-center rounded-full bg-red-100 text-red-500 hover:bg-red-500 hover:text-white transition-colors cursor-pointer">
                                ✕
                            </button>
                        </div>
                        ...
                        <div className="flex gap-2 items-center mt-1">
                            <div className="flex-1 flex flex-col">
                                ...
                                <input 
                                    type="number" 
                                    min={isPieza ? "1" : "0"} 
                                    step={isPieza ? "1" : "0.001"} 
                                    value={item.quantity ?? ""} 
                                    onChange={e => updateCartItem(item.product.id, 'quantity', e.target.value)} 
                                    className="w-full rounded-md border border-gray-300 px-2 py-1 text-xs font-bold text-center outline-none focus:border-primary" 
                                />
                            </div>
                            <span className="text-gray-300 font-bold mt-3">x</span>
                            <div className="flex-1 flex flex-col">
                                <label className="text-[9px] font-bold text-gray-400 uppercase">P. Final ($)</label>
                                <input 
                                    type="number" 
                                    min="0" 
                                    step="0.01" 
                                    value={item.finalPrice ?? ""} 
                                    onChange={e => updateCartItem(item.product.id, 'finalPrice', e.target.value)} 
                                    disabled={!isAdmin} 
                                    className="w-full rounded-md border border-gray-300 px-2 py-1 text-xs font-bold text-center outline-none focus:border-primary disabled:bg-gray-100 disabled:text-gray-500 disabled:border-transparent" 
                                />
                            </div>
                            <span className="text-gray-300 font-bold mt-3">=</span>
                            <div className="flex-[1.2] flex flex-col items-end pt-3">
                                <span className="font-extrabold text-primary text-sm">${((Number(item.quantity) || 0) * (Number(item.finalPrice) || 0)).toFixed(2)}</span>
                            </div>
                        </div>
                    </div>
                )
          ...
          <div className="bg-gray-50 p-4 border-t border-gray-200 shrink-0">
            <div className="flex justify-between items-center mb-4">
                ...
                <span className="text-3xl font-extrabold text-gray-900">${(cartTotal || 0).toFixed(2)}</span>
            </div>

            <div className="flex gap-2 mb-4">
                <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-colors ${isCash ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-100'}`}>
                    <input type="radio" checked={isCash} onChange={() => setIsCash(true)} className="hidden" />
                    <span className="text-xl">💵</span> <span className="font-bold text-sm">Efectivo</span>
                </label>
                <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-xl border-2 cursor-pointer transition-colors ${!isCash ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-100'}`}>
                    <input type="radio" checked={!isCash} onChange={() => setIsCash(false)} className="hidden" />
                    <span className="text-xl">💳</span> <span className="font-bold text-sm">Tarjeta</span>
                </label>
            </div>

            <button disabled={cart.length === 0 || isProcessing} onClick={handleCheckout} className="w-full rounded-xl bg-primary px-4 py-4 text-sm font-bold text-white hover:brightness-90 transition-all shadow-lg shadow-primary/30 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer uppercase tracking-widest">
              {isProcessing ? "Procesando..." : "Aplicar Compra"}
            </button>
          </div>
        </div>
      </div>
        ...
        {errorModal.isOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg flex flex-col gap-4 border border-gray-100"
            >
              ...
              <div className="bg-red-50/50 p-4 rounded-xl border border-red-100 max-h-64 overflow-y-auto custom-scrollbar">
                <pre className="text-xs text-red-800 whitespace-pre-wrap font-mono break-words">
                  {errorModal.details}
                </pre>
              </div>
              ...
              <div className="flex justify-end pt-2">
                <button
                  onClick={() => setErrorModal({ isOpen: false, title: "", details: "" })}
                  className="bg-gray-900 text-white px-6 py-2.5 rounded-xl font-bold text-sm hover:bg-gray-800 transition-colors shadow-lg cursor-pointer"
                >
                  Cerrar
                </button>
              </div>
