...
export function History() {
  ...
  const handleOpenDetails = async (sale: Sale) => {
    ...
    try {
      ...
      setToast({ msg: "Error al cargar ticket" });
      setTimeout(() => setToast(null), 3500);
    } finally {
      setLoading(false);
    }
      ...
      <header className="flex items-center justify-between rounded-xl bg-white border border-gray-200 px-4 py-3 shadow-sm shrink-0 flex-wrap gap-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <h1 className="text-lg sm:text-xl font-extrabold text-gray-900 leading-none">Supervisión de Ventas</h1>
          </div>
        </div>
        
        <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200">
          <button onClick={() => setActiveTab('history')} className={`px-4 py-2 text-xs font-bold rounded-md transition-colors cursor-pointer ${activeTab === 'history' ? 'bg-white shadow-sm text-primary' : 'text-gray-500 hover:text-gray-800'}`}>
            Historial de Tickets
          </button>
          <button onClick={() => setActiveTab('analytics')} className={`px-4 py-2 text-xs font-bold rounded-md transition-colors cursor-pointer ${activeTab === 'analytics' ? 'bg-white shadow-sm text-primary' : 'text-gray-500 hover:text-gray-800'}`}>
